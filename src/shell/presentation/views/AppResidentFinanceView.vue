...
const fees = computed(() => {
  const baseFees = paymentState.fees || []
  const settings = paymentState.settings
  if (!settings) return baseFees
  
  return baseFees.map(f => {
    // Only apply live mora to unpaid items
    if (f.status === 'Pagado') return f
    
    const days = getDaysUntilDue(f.dueDate)
    // If overdue by date but lateFee is still 0 in DB AND it has not been waived, calculate estimated one
    if (days < 0 && (f.lateFee || 0) === 0 && !f.isLateFeeWaived) {
      const rate = settings.lateFeeRate || 0.05
      const estimatedMora = Math.round(f.baseAmount * rate * 100) / 100
      return {
        ...f,
        lateFee: estimatedMora,
        amount: (f.baseAmount || 0) + estimatedMora + (f.extraCharges || 0)
      }
    }
    return f
  })
                  ...
                  <template #body="{ data }">
                    <div class="flex flex-column">
                      <span class="font-semibold text-900">
                        S/ {{ data.baseAmount.toFixed(2) }}
                        <span v-if="data.lateFee > 0" class="text-red-500 ml-1 animate-pulse">
                          + S/ {{ data.lateFee.toFixed(2) }}
                        </span>
                      </span>
                      <small v-if="data.lateFee > 0" class="text-red-500 font-medium">
                        {{ t('financeAdmin.lateFeeLabel', 'Mora aplicada por retraso') }}
                      </small>
                    </div>
                  </template>
                </Column>
