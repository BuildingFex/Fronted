...
export const financesApi = {
  ...
  async updateSettings(settingsId, updateData) {
    const { data } = await apiClient.patch(`/financeSettings/${settingsId}`, updateData)
    return data
  },
  ...
  async getFees(residentId) {
    ...
    const mappedReceipts = receiptsData.map(receipt => {
      ...
      return {
        ...
        baseAmount: Number(receipt.amount),
        lateFee: Number(receipt.lateFee || 0),
        extraCharges: Number(receipt.extraCharges || 0),
        ...
        isLateFeeWaived: !!receipt.isLateFeeWaived,
